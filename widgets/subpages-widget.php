<?php
/**
 * Widget Name: F4D: Subpages Widget
...
class Subpages_Widget extends WP_Widget {
	...
	public function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_subpages', 'description' => esc_html__('Display subpages if present page only.', 'f4d') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'subpages-widget' );

		/* Create the widget. */
		parent::__construct( 'subpages-widget', esc_html__('F4D: Subpages', 'f4d'), $widget_ops, $control_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		/**
		 * Filter the arguments for the 'F4D: Subpages' widget.
		 *
		 * @since 2.7.5
		 *
		 * @see wp_list_pages()
		 ...
		 */
		$children = wp_list_pages( apply_filters( 'f4d_get_subpages_widget_args', array(
			'title_li'    => '',
			'echo'        => 0,
			'sort_column' => $sortby,
			'exclude'     => $exclude,
...
<?php		    /* After widget (defined by themes). */
	...
	public function form( $instance ) {
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e('Title:', 'f4d'); ?></label>
		    ...
		    <br />
		    <?php esc_html_e('Leave the Title field blank if you would like to display the parent page Title instead.', 'f4d'); ?>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('sortby'); ?>"><?php _e( 'Sort by:', 'f4d' ); ?></label>
			<select name="<?php echo $this->get_field_name('sortby'); ?>" id="<?php echo $this->get_field_id('sortby'); ?>" class="widefat">
				<option value="post_title"<?php selected( $instance['sortby'], 'post_title' ); ?>><?php _e('Page title', 'f4d'); ?></option>
				<option value="menu_order"<?php selected( $instance['sortby'], 'menu_order' ); ?>><?php _e('Page order', 'f4d'); ?></option>
				<option value="ID"<?php selected( $instance['sortby'], 'ID' ); ?>><?php _e( 'Page ID', 'f4d' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('exclude'); ?>"><?php _e( 'Exclude:', 'f4d' ); ?></label> <input type="text" value="<?php echo $exclude; ?>" name="<?php echo $this->get_field_name('exclude'); ?>" id="<?php echo $this->get_field_id('exclude'); ?>" class="widefat" />
			<br />
			<small><?php _e( 'Page IDs, separated by commas.', 'f4d' ); ?></small>
		</p>
<?php
	}
}
